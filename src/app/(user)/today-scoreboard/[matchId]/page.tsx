...
export default function ScoreboardPage() {
  ...
  const fetchMatch = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/today-scoreboard/${matchId}`, { cache: 'no-store' });
      ...
      if (!data.match.referee_id && !claimAttempted.current && !data.isReferee) {
        claimAttempted.current = true;
        try {
          const claimRes = await fetch('/api/match-referee', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ schedule_id: matchId, action: 'claim' })
          });
          if (claimRes.ok) {
            // Claim successful, refetch to get updated referee status
            const refetchRes = await fetch(`/api/today-scoreboard/${matchId}`, { cache: 'no-store' });
            const refetchData = await refetchRes.json();
            if (refetchRes.ok) {
              setMatch(refetchData.match);
              setCanEdit(refetchData.canEdit);
              setIsReferee(refetchData.isReferee);
              setIsAdmin(refetchData.isAdmin ?? false);
              setScore1(refetchData.match.score_team1 ?? 0);
              setScore2(refetchData.match.score_team2 ?? 0);
              setError(null);
              setLoading(false);
              return;
            }
          ...
          console.error('Failed to auto-claim referee', e);
        }
      }
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`today-scoreboard-${matchId}`)
      .on(
        ...
        (payload: any) => {
          ...
          if (newData) {
            const result = newData.match_result || {};
            setMatch((prev) =>
              prev
                ? {
                    ...prev,
                    score_team1: result.team1_score ?? prev.score_team1,
                    score_team2: result.team2_score ?? prev.score_team2,
                    status: newData.status ?? prev.status,
                    winner: result.winner ?? prev.winner,
                  }
                : prev
            );
            ...
            // 관전 모드에서는 점수도 갱신
            if (!canEdit) {
              setScore1(result.team1_score ?? 0);
              setScore2(result.team2_score ?? 0);
            }
          }
        }
      )
